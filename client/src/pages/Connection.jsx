...
{/* 
        ...
        <div className='mb-8'>
          ...
          <p className='text-slate-600'>Manage your network and discover new connnections</p>
        ...
        rounded-md p-1 bg-white shadow-sm'>
          {
            dataArray.map((tab)=>(
              ...
              className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors  ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500hover:text-black'}`}>
                <tab.icon className='w-4 h-4'/>
                <span className='ml-1'>{tab.label}</span>
                {tab.count !== undefined && (
                  <span className='ml-2 tex-xs bg-gray-100 text-gray-700 px-2 py-0.5
                  rounded-full'>{tab.label}</span>
                )}
                </button>
            ))
          }
